...
function initPieceFactory () {
	...
	var colors = [];
	for(var c = 0; c<2; c++) {
		colors[c] = textures['img/texture/wood-'+c+'.jpg'].clone();
	...
	function createPiece(name,color) {
		...
		var urlJson = 'resources/3D/json/'+name+'.json';
		var urlAO   = 'img/texture/'+name+'-ao.jpg';
...
function createFloor(size,chessboardSize) {
	...
	var diff  = textures['img/texture/floor.jpg'];
	var spec  = textures['img/texture/floor_S.jpg'];
	var norm  = textures['img/texture/floor_N.jpg'];
	var light = textures['img/texture/fakeShadow.jpg'];
